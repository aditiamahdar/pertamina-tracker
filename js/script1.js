  ...
  sess.updateDataFlags(
    ...
    function (code) { if (code) return;
      ...
      for(var i=0; i<un.length; i++)
        if (un[i].getName() == mt) {  // filter only selected unit
...
function initMap(session) {
  ...
  center = {
    ...
    lng: parseFloat(center[1]) || -6.1861726,
    // lat: parseFloat(center[0]) ||  28.49,
    lat: parseFloat(center[0]) || 106.8172804,
...
function changeTime(value) {
  ...
  // var tt = new Date("2020-01-06T03:00:00");
  // var tnow = new Date("2020-01-06T06:00:00");

  var tt = new Date(get_html_var("from"));
  var tnow = new Date(get_html_var("to"));
...
function ltranslate(){
  var title = decodeURIComponent(APP_CONFIG.alias || "FleetSight");
  ...
  // intervals += "<option value='6' selected='selected'>" + $.localise.tr("Custom interval") + "</option>";
  var intervals = "<option value='6' selected='selected'>" + $.localise.tr("Custom interval") + "</option>";
